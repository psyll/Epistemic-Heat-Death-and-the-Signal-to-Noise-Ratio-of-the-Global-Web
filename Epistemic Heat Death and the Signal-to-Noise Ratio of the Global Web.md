...
*v2.1 change note: this revision computationally re-verifies every quantitative claim in Section 5 against the reference implementation in `scripts/`, regenerates all ten in-text figures directly from that code (none previously existed), and independently fact-checks the empirical anchors in Section 9 and Section 11 against primary and independent secondary sources as of July 2, 2026 - correcting one arithmetic error, removing one citation that could not be substantiated, and adding two new corroborating data points (see the new Appendix J for the full record). It does not claim empirical confirmation of the framework's own theoretical claims, which remain open per Section 9.5 and Appendix C - see Appendix J.4 for why that distinction matters and is being drawn explicitly.*
...
Graphite (cited in Axios, 2025, October 14; updated Axios, 2026, May 15). AI-written web pages haven't overwhelmed human-authored content, study finds / AI now writes as many online articles as humans do. Analysis of URLs (2020-2025, extended through March 2026) using AI-detector tooling against Common Crawl data.
